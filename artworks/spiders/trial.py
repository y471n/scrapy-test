...
class TrialSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        category_heading = parent_heading + "," + category_heading
        ...
        next_link = response.xpath(self._xpath_selectors.get("next_link"))
        yield response.follow(next_link, self.parse, meta={"parent_heading": category_heading})

    def parse_item(self, response):
        loader = ArtworksLoader(ArtworksItem(), response=response)
